...
public class BinHexInfo {
	...
	private static void process(File src, boolean roundTripTest) {
		try {
			...
			BinHexInputStream hqx = new BinHexInputStream(in);
			...
			if (roundTripTest) try {
				...
				BinHexOutputStream out = new BinHexOutputStream(sb);
				out.writeAppleFile(null, af);
				out.close();
				BinHexInputStream in2 = new BinHexInputStream(sb.toString());
				AppleFile af2 = in2.readAppleFile();
				in2.close();
	...
	private static final int[] PARTS = {
		...
		AppleFilePart.TYPE_DATA_FORK, AppleFilePart.TYPE_RESOURCE_FORK
	};
}
